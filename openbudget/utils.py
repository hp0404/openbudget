...
def get_cumulative(
    files: List[Dict[str, Any]], item_type: str = "EXPENSES"
...
def transform(d: Dict[int, pd.DataFrame], item_type: str, year: str) -> pd.DataFrame:
    ...
    for current, previous in zip(current_range, previous_range):
        if previous != 0:
            ...
            tmp = (
                d[current]
                .set_index(idxs)
                .subtract(d[previous].set_index(idxs), fill_value=0)
            )
            ac = d[current].set_index(idxs)["ADJUSTED"]
            # cumulative
